$(function() {
	function e() {
		...
		//获取cookie中存储的skin皮肤的颜色
		var skinColor = $.cookie("skin-color");
		//获取cookie中存储的slidebar部分的颜色
		var sidebarColor = $.cookie("sidebar-color");
		//获取cookie中存储的navbar部分的颜色
		var navbarColor = $.cookie("navbar-color");
		if (skinColor) {
			$("body").addClass("skin-" + skinColor)
		}
		if (sidebarColor) {
			$("#main-container").addClass("sidebar-" + sidebarColor)
		}
		if (navbarColor) {
			$("#navbar").addClass("navbar-" + navbarColor)
		}
	}
	e();
//	将属性是以#开头的a标签的单击事件，阻止掉默认的行为
...
//	一个容器中内容滚动的动画
	if ($(".tile-active").length > 0) {
		...
		$(".tile-active").each(function(e, t) {
			var firstNode, lastNode, firstNodeStop, lastNodeStop, firstNodeOuterHeight;
			firstNode = $(this).children(".tile").first();
			lastNode = $(this).children(".tile").last();
			firstNodeStop = $(firstNode).data("stop");
			lastNodeStop = $(lastNode).data("stop");
			firstNodeOuterHeight = $(firstNode).outerHeight();
			if (firstNodeStop == undefined) {
				firstNodeStop = o
			}
			if (lastNodeStop == undefined) {
				lastNodeStop = o
			}
			setTimeout(function() {
				u(t, firstNodeStop, lastNodeStop, firstNodeOuterHeight)
			}, firstNodeOuterHeight)
		})
	}
